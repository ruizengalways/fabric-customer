...
from .engine import materialize_scenario, replay_day, reset_scenario
...
def _parser() -> argparse.ArgumentParser:
    ...
    materialize = sub.add_parser("materialize", help="Generate source deliveries and expected truth")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "materialize":
        root = materialize_scenario(Path(args.output), through_day=args.through_day, clean=not args.no_clean)
        print(root)
        return 0
